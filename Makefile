...
.PHONY: test-db-up test-db-down test-db-reset test-migrate-up test-integration test run build dev
...
test-migrate-up:
	DATABASE_URL="$(TEST_DB_URL)" goose -dir ./migrations postgres "$(TEST_DB_URL)"
